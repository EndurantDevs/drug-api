...
def _require_handoff_engine(database) -> None:
    ...
    if factory is not None and (
        not isinstance(factory, async_sessionmaker)
        or not isinstance(database.engine, AsyncEngine)
...
def _candidate_digest(receipt_dict: dict) -> str:
    candidate_dict = {
        key: entry for key, entry in receipt_dict.items() if key not in {"handed_off_at", "handoff_sha256"}
    }
    encoded = json.dumps(
        candidate_dict, sort_keys=True, separators=(",", ":"), ensure_ascii=False, allow_nan=False
    ).encode()
...
def _has_valid_tables(receipt_dict: dict) -> bool:
    ...
    for name, table_dict in tables_by_name.items():
        if (
            set(table_dict) != {"name", "oid", "row_count", "size_bytes", "sha256", "encoding", "columns"}
...
def _has_valid_columns(columns) -> bool:
    ...
    for column_dict in columns:
        if (
            not isinstance(column_dict, dict)
            or set(column_dict) != {"name", "type", "not_null"}
            or not isinstance(column_dict["name"], str)
            or not column_dict["name"]
            or not isinstance(column_dict["type"], str)
            or not column_dict["type"]
...
def has_valid_ndc_handoff(receipt_dict, run_id: str, attempt_id: str | None = None) -> bool:
    ...
    try:
        if (
            not isinstance(receipt_dict, dict)
            or set(receipt_dict) != _FIELDS
            or receipt_dict["format"] != HANDOFF_FORMAT
            or receipt_dict["run_id"] != run_id
            ...
            or receipt_dict["complete"] is not True
            or receipt_dict["published"] is not False
        ):
            return False
        if not all(_is_positive_oid(receipt_dict[key]) for key in ("database_oid", "import_run_oid")):
            return False
        counts_by_name = receipt_dict["counts"]
        if set(counts_by_name) != _COUNTS or any(
            type(count) is not int or count < 0 for count in counts_by_name.values()
        ):
            return False
...
async def _handoff_receipt(database, attempt, audit_dict: dict) -> dict:
    tables_by_name = {
        name: {**table_dict, "name": attempt.tables[name].name} for name, table_dict in audit_dict["tables"].items()
    }
    ...
    receipt_dict = {
        **audit_dict,
        ...
        "import_run_oid": await database.scalar(
            text("SELECT to_regclass(:name)::oid"), name=f"{attempt.schema}.import_run"
        ),
        "tables": tables_by_name,
        "incumbent_oids": dict(attempt.incumbent_oids),
        "published": False,
...
async def handoff_ndc_stages(database, attempt, audit_dict: dict) -> dict:
    ...
    """),
        ...
        progress=json.dumps(
            {
                "unit": "records",
                "done": attempt.counts["source_products"],
                "total": attempt.counts["source_products"],
                "pct": 100,
    ...
    marker = json.dumps(
        {
            ...
            "run_id": attempt.run_id,
            "attempt_id": attempt.suffix,
            "handoff_sha256": receipt_dict["handoff_sha256"],
        },
        sort_keys=True,
    )
