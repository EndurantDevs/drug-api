"""Durable publication identity shared by native imports and archive workers."""
...
async def require_label_ordinary_publication(database: Any, schema: str) -> None:
    """Reject a protected live heap before an ordinary finalizer performs any DDL."""
    row = await _first(
        database,
        text(
            "SELECT NOT EXISTS (SELECT 1 FROM pg_catalog.pg_class c "
            "JOIN pg_catalog.pg_namespace n ON n.oid=c.relnamespace "
            "WHERE n.nspname=:schema AND c.relname='label' "
            "AND NOT pg_catalog.pg_has_role(current_user,c.relowner,'USAGE')) AS allowed"
        ),
        schema=schema_name(schema),
    )
    if _row_mapping(row).get("allowed") is not True:
        raise RuntimeError("ordinary Label publication is disabled for the protected live table")
...
async def adopt_label_generation(
    database: Any, *, schema: str, source_generation: object
) -> ResultPublicationAuthority:
    """Bind a validated foreign generation to local OIDs inside the cutover transaction."""

    schema = schema_name(schema)
    incoming = validate_serving_generation(source_generation)
    current = await read_result_publication_authority(database, importer_id="label", schema=schema, lock=True)
    if current.local_generation >= _MAX_GENERATION:
        raise RuntimeError("result publication local generation is exhausted")
    identities = await relation_identities(database, schema, ("label",))
    updated = await _first(
        database,
        text(
            f'UPDATE "{schema}"."{TABLE_NAME}" SET local_generation=local_generation+1, '
            "origin_lineage_id=CAST(:lineage AS uuid), origin_generation=:generation, "
            "published_at=CAST(:published_at AS timestamptz), relation_oids=CAST(:oids AS bigint[]), "
            "consumed_dependencies='{}'::jsonb WHERE importer_id='label' "
            "RETURNING importer_id,local_lineage_id,local_generation,origin_lineage_id,origin_generation,"
            "published_at,relation_oids,consumed_dependencies"
        ),
        lineage=incoming.origin_lineage_id,
        generation=incoming.origin_generation,
        published_at=incoming.published_at,
        oids=[oid for _, oid in identities],
    )
    return validate_result_publication_authority(updated)


__all__ = [
    "adopt_label_generation",
    "DEPENDENCY_FORMAT",
    "DEPENDENCY_RELATIONS",
    "RELATION_NAMES_BY_IMPORTER",
    "ResultPublicationAuthority",
